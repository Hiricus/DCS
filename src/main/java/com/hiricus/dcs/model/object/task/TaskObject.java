...
public class TaskObject {
    ...
    public TaskObject(String name, TaskType type, Integer relatedGroupId) {
        this.name = name;
        this.type = type;
    ...
    public TaskObject(Record record) {
        ...
        this.isCompleted = record.get(TASK.IS_COMPLETED);
        this.hasDocuments = record.get(TASK.HAS_DOCUMENTS);
    }

    // Не передаёт информацию о группе
    public TaskObject(TaskDto taskDto) {
        this.id = taskDto.getId();
        this.name = taskDto.getName();
        this.type = TaskType.valueOf(taskDto.getType());
        this.isCompleted = taskDto.getIsCompleted();
        this.hasDocuments = taskDto.getHasDocuments();
    }
